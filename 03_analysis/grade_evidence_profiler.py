...
def render_latex_table(profile_df: pd.DataFrame, extraction_path: Path) -> str:
    rows = [
        [
            ...
            normalize(row.get("risk_of_bias", "")),
            normalize(row.get("inconsistency", "")),
            normalize(row.get("indirectness", "")),
            normalize(row.get("imprecision", "")),
            normalize(row.get("overall_certainty", "")),
        ]
        for _, row in profile_df.iterrows()
    ]

    return render_table_block(
        generator_name="03_analysis/grade_evidence_profiler.py",
        source_path=extraction_path,
        caption="GRADE evidence profile by included study (auto-generated)",
        label="tab:grade_evidence_profile",
        column_spec=(
            r"p{0.11\textwidth}p{0.14\textwidth}p{0.11\textwidth}"
            r"p{0.11\textwidth}p{0.11\textwidth}p{0.11\textwidth}p{0.15\textwidth}"
        ),
        headers=[
            "Study",
            "Design",
        ...
        rows=rows,
        empty_row_latex=(
            r"\multicolumn{7}{p{0.93\textwidth}}"
            r"{No included studies with non-empty study\_id are available for GRADE profiling yet.} \\"
        ),
        pre_tabular_lines=[
