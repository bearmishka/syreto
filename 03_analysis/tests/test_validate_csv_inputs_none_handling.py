...
class ValidateCsvInputsNoneHandlingTests(unittest.TestCase):
    ...
    def test_cli_uses_script_relative_paths_when_called_outside_analysis_dir(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            summary_path = Path(tmp_dir) / "csv_input_validation_summary.md"
            ...
            self.assertIn("- Errors: 0", summary_text)
            self.assertNotIn("CSV file is missing.", summary_text)
