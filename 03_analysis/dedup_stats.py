...
def render_prisma_flow_tikz(layout: FlowLayout, output_path: Path, *, flow_style: str = "default") -> None:
    ...
    lines: list[str] = [
        ...
        (
            ...
            f"{{{node_text('Reports assessed for eligibility\n(full text)', count_by_key.get('full_text_assessed'))}}};"
        ),
        (
            f"\\node[prismaMain,anchor=north] (qualitative) at ({positions['qualitative'][0]:.2f},{positions['qualitative'][1]:.2f}) "
            f"{{{node_text('Studies included in\nqualitative synthesis', count_by_key.get('included_qualitative'))}}};"
        ),
        (
            f"\\node[prismaMainSoft,anchor=north] (quantitative) at ({positions['quantitative'][0]:.2f},{positions['quantitative'][1]:.2f}) "
            f"{{{node_text('Studies included in\nquantitative synthesis', count_by_key.get('included_quantitative'))}}};"
        ),
        (
        ...
        (
            ...
            f"{{{node_text('Records excluded\n(title/abstract)', count_by_key.get('excluded_title_abstract'))}}};"
        ),
        (
            f"\\node[prismaSide,anchor=north west] (excluded_ft) at ({positions['excluded_ft'][0]:.2f},{positions['excluded_ft'][1]:.2f}) "
            f"{{{node_text('Reports excluded\n(full text)', count_by_key.get('excluded_full_text'))}}};"
        ),
...
def compute_title_abstract_prisma_stats(results_df: pd.DataFrame) -> dict[str, int | bool]:
    ...
    metrics["reports_sought_for_retrieval_from_title_abstract"] = int(working["decision_norm"].eq("include").sum())
    return metrics
...
def compute_stats(
    ...
    if not search_df.empty and "results_total" in search_df.columns:
        identified_from_search_log = int(pd.to_numeric(search_df["results_total"], errors="coerce").fillna(0).sum())

    identified_databases = identified_from_search_log if identified_from_search_log > 0 else total_rows
    ...
    reports_sought_for_retrieval = int(title_abstract_stats["reports_sought_for_retrieval_from_title_abstract"])
    ...
    stats: dict[str, int | bool] = {
        ...
        "records_excluded_title_abstract": int(title_abstract_stats["records_excluded_title_abstract"]),
        ...
        "title_abstract_results_present": bool(title_abstract_stats["title_abstract_results_present"]),
        ...
        mapping.update(
            {
                ...
                "studies_included_qualitative_synthesis": int(stats["studies_included_qualitative_synthesis"]),
            }
        )
...
def prisma_sync_deltas(before_df: pd.DataFrame, after_df: pd.DataFrame) -> list[tuple[str, int | None, int | None]]:
...
def build_summary(
    ...
    *,
    ...
    lines.append(f"- Studies included (qualitative): {stats['studies_included_qualitative_synthesis']}")
    lines.append("")
    lines.append("## Diagnostics")
    lines.append("")
    lines.append(f"- Total non-empty rows in `master_records.csv`: {stats['total_rows_in_master']}")
    lines.append(f"- Sum of `results_total` in `search_log.csv`: {stats['identified_from_search_log']}")
    lines.append(f"- Title/abstract consensus rows (unique record_id): {stats['title_abstract_records_unique']}")
    ...
    lines.append("- If `search_log.csv` totals are empty/zero, fallback uses `master_records.csv` row count.")
    if bool(stats.get("title_abstract_results_present")):
        lines.append("- Title/abstract PRISMA counts are synced from `screening_title_abstract_results.csv`.")
    else:
        lines.append(
    ...
    if bool(stats.get("fulltext_log_present")):
        ...
        lines.append("- `screening_fulltext_log.csv` has no usable rows; existing PRISMA full-text counts are kept unchanged.")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Compute deduplication stats and update PRISMA count template.")
    ...
    parser.add_argument("--summary", default="outputs/dedup_stats_summary.md", help="Path to markdown summary output")
    parser.add_argument(
        "--flow-output",
    ...
    fulltext_df = pd.read_csv(screening_fulltext_log_path) if screening_fulltext_log_path.exists() else pd.DataFrame()
    ...
    if args.apply:
        backup_path = Path(args.backup_path) if args.backup_path else default_backup_path(prisma_path)
    ...
    if args.apply:
        ...
        print("Dry-run: PRISMA counts preview generated; no changes written (use --apply --backup).")
