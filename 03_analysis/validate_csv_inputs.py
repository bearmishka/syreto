...
RAW_EXPORTS_DIR = resolve_input_path(RAW_EXPORTS_DIR)
...
def validate_file(spec: dict, *, template_filename_as_info: bool = True) -> tuple[dict, list[dict]]:
    ...
    path = resolve_input_path(spec_path)
    display_path = spec_path.as_posix()
    file_name = spec["name"]
...
def main() -> None:
    ...
    if args.migrate_master_records:
        ...
        if master_spec is not None:
            _, migration_message = migrate_master_records_abstract_column(
                resolve_input_path(Path(master_spec["path"]))
            )
